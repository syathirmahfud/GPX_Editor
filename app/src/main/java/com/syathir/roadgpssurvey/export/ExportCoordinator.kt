...
class ExportCoordinator(
    ...
    suspend fun export(sessionId: Long, format: ExportFormat, destination: Uri) = withContext(Dispatchers.IO) {
        ...
        output.use { stream ->
            when (format) {
                ExportFormat.CSV -> OutputStreamWriter(stream, Charsets.UTF_8).use { writer -> CsvExporter().write(data, writer) }
                ExportFormat.GPX -> OutputStreamWriter(stream, Charsets.UTF_8).use { writer -> GpxExporter().write(data, writer) }
                ExportFormat.KMZ -> KmzExporter().write(data, stream)
            }
        }
    }
}
